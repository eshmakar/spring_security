...
public class MySecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {//перезаписываем только этот метод
        auth.jdbcAuthentication().dataSource(dataSource);//с этим объясняем Spring-у, что необходимо брать информацию о юзерах из базы данных (настройки подлкючения находятся в dataSource)

        //ДАННЫЕ ЮЗЕРОВ УЖЕ БУДУТ ХРАНЯТСЯ В БАЗЕ ДАННЫХ
